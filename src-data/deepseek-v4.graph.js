window.DSV4_GRAPH = (() => {
  ...
  Object.entries({
    "input-ids": { why: "input_ids는 [B,S] forward chunk로 들어와 토큰 임베딩의 원천이 되고, 초반 hash routing에서는 tid2eid lookup에 직접 쓰이는 lexical prior 역할도 합니다. 다만 자주 등장하는 토큰이 특정 expert로 쏠리는 문제를 어떻게 완화했는지는 공개 코드만으로는 보이지 않습니다.", runtime: "hash routing 레이어는 score top-k로 expert id를 고르지 않고 input_ids.flatten()으로 tid2eid를 조회합니다.", ui: "임베딩 입력과 초반 라우팅 메타데이터라는 두 용도를 같이 보여주는 게 좋습니다.", open: "tid2eid가 학습/배정될 때 자주 등장하는 토큰의 expert 쏠림을 어떻게 막았는지는 공개 inference 코드만으로는 확인되지 않습니다." },
    embedding: { why: "이산 token id를 모든 sublayer가 공유할 연속 표현으로 바꾸는 표준 진입점입니다. tensor parallel에서는 vocab shard별 partial embedding을 만든 뒤 all-reduce로 같은 hidden 표현을 맞추기 때문에, decoder stack 안에서 반복 실행되는 노드가 아니라 stack 진입 전 1회 수행되는 입력 변환으로 보는 편이 정확합니다.", runtime: "tensor parallel에서는 vocab shard 밖 id를 mask하고 partial embedding을 all-reduce합니다.", ui: "decoder stack 안에서 매번 실행되는 노드가 아니라 stack 진입 전 1회 노드로 표시합니다." },
    "hc-expand": { why: "단일 residual stream을 [B,S,4,D]의 4개 lane으로 복제해 mHC가 layer마다 read/write/mixing을 선택할 수 있는 상태 공간을 만듭니다. mHC는 residual connection을 강화하고 layer 간 signal propagation을 안정화하려는 구조이며, lane별 역할 분화는 구조상 가능한 해석입니다.", runtime: "공식 forward는 [B,S,D]를 [B,S,hc_mult,D]로 repeat합니다.", ui: "lane 축을 실제 tensor 차원으로 보여주고, 모델 복사본 4개처럼 보이지 않게 합니다." },
    "mhc-attn": { why: "attention sublayer가 residual lane 전체를 직접 받는 대신 controller가 pre/post/comb를 만들고, data path가 read projection으로 필요한 stream을 읽은 뒤 write projection으로 다시 lane에 분배합니다. attention 주변에서 read/write path가 분리되기 때문에 residual 전달을 안정화하면서 attention update가 들어갈 위치도 조절할 수 있습니다.", runtime: "controller coefficient는 inference에서도 현재 residual lane에서 매번 다시 계산됩니다.", ui: "attention 자체가 아니라 attention 주변의 controller + data path로 표시합니다." },
    attention: { why: "긴 context에서 모든 과거 token을 dense attention으로 보지 않고 SWA, compressed attention, sparse retrieval을 layer별로 조합합니다. R=4 layer는 Lightning Indexer를 써 compressed block을 고르고, R=128 layer는 더 강하게 압축된 dense compressed path를 쓰면서 long-context memory와 compute를 줄입니다.", runtime: "내부 cache path는 layer ratio에 따라 CSA/c4a R=4, HCA/c128a R=128, SWA-only R=0으로 달라집니다.", ui: "선택된 한 layer에서 CSA와 HCA가 동시에 켜진 것처럼 보이지 않게 합니다." },
    "q-path": { why: "query 생성을 low-rank latent 단계와 head expansion 단계로 나눠 큰 H*512 projection을 직접 다루는 부담을 줄입니다. q_lora_rank와 wq_a/wq_b 분리 덕분에 query projection 비용을 낮추고, 마지막 64 dimension에만 RoPE를 적용해 positional slice를 분리합니다.", ui: "compact scene에서만 쓰고, overview에서는 하위 노드들을 직접 보여주는 편이 좋습니다." },
    "kv-path": { why: "head마다 별도 KV를 저장하지 않고 하나의 [B,S,512] shared KV를 key/value 양쪽에 재사용해 cache memory를 크게 줄입니다. shared KV를 쓰면 cache memory는 줄지만 key score에 필요한 RoPE phase가 value path에 남을 수 있어, value로 사용할 때는 inverse RoPE 보정이 필요합니다.", runtime: "동일한 shared vector가 logit 계산에서는 key처럼, output 계산에서는 value처럼 쓰입니다.", ui: "KV sharing과 inverse RoPE 이야기가 시작되는 핵심 노드입니다." },
    "kv-cache": { why: "최근 128개 uncompressed SWA entry와 오래된 compressed entry를 같은 attention id 공간에 놓아 kernel indexing을 단순화합니다. prefill과 decode의 write path는 다르지만, 이 layout 덕분에 long-context memory를 줄이면서 최근 local detail은 uncompressed cache로 보존합니다.", runtime: "앞 128 slot은 SWA, 그 뒤 suffix는 compressed entry입니다. decode에서는 SWA를 start_pos % 128 위치에 씁니다.", ui: "window prefix + compressed suffix 구조로 그립니다." },
    compressor: { why: "1M context의 오래된 KV를 모두 원본으로 유지하지 않고 compressed entry로 바꿔 long-context memory를 줄입니다. R=4에서는 overlap을 둔 c4a 방식으로 더 촘촘한 compressed entry를 만들고, R=128에서는 non-overlap c128a 방식으로 훨씬 강하게 압축합니다.", runtime: "R=4는 c4a overlap, R=128은 c128a non-overlap 동작입니다.", ui: "선택된 layer mode에 따라 c4a/c128a label을 보여줍니다." },
    indexer: { why: "c4a의 T/4 compressed blocks가 여전히 많기 때문에 attention 전에 중요한 compressed block만 고르는 cheap retrieval path를 둡니다. Hadamard rotation, FP4 activation quantization, weighted ReLU score로 ranking 비용을 낮춘 뒤 후보 block을 좁혀 long-context attention compute를 줄입니다.", runtime: "Pro topK는 1024, Flash topK는 512입니다.", ui: "CSA/c4a 모드에서만 보여줍니다." },
    "sparse-attn": { why: "SWA, CSA, HCA의 후보 선택 차이를 하나의 selected-KV attention kernel로 실행합니다. N은 mode에 따라 최근 window만 보거나, window에 topK compressed entry를 더하거나, valid compressed block 전체를 더하는 식으로 달라지며, 같은 kernel 추상화 안에서 long-context compute와 memory를 줄이고 local 정보는 유지합니다.", runtime: "per-head attn_sink와 online-softmax 스타일 kernel 동작을 포함합니다.", ui: "선택 모드별 후보 수를 <=128, topK+128, T/128+128처럼 보여줍니다." },
    "o-proj": { why: "sparse attention head 결과를 residual stream 크기로 되돌리면서 grouped low-rank output projection으로 비용을 낮춥니다. 각 group은 8 heads * 512 dims 단위로 접히며, wo_a/wo_b 구조가 projection 비용을 낮추고 inverse RoPE로 보정된 value semantics를 residual stream에 맞춥니다.", runtime: "KV가 shared라서 grouped low-rank output projection 전에 inverse RoPE를 적용합니다.", ui: "shared-KV attention 이후 value semantics를 보정하는 지점입니다." },
    "mhc-ffn": { why: "MoE sublayer도 attention과 같은 mHC read/write 구조로 감싸 residual lane 안정성을 유지합니다. FFN 쪽 mHC는 attention mHC와 목적은 같지만 별도 parameter set을 쓰므로, MoE 입력 읽기와 MoE output writeback 정책을 독립적으로 학습할 수 있습니다.", runtime: "attention mHC와 별도의 FFN mHC parameter set을 씁니다." },
    moe: { why: "전체 parameter capacity는 크게 유지하되 token당 활성화 expert 수를 top-6로 제한해 inference compute를 줄입니다. routed experts는 조건부로 선택되고 shared expert는 항상 더해지며, 첫 3개 layer에서는 score top-k 대신 hash routing을 사용합니다.", runtime: "공개 구현에서는 모든 decoder block이 MoE FFN을 씁니다.", ui: "shared expert는 항상 active, routed expert는 conditional로 그립니다." },
    gate: { why: "token별로 필요한 expert subset을 선택해 sparse FFN을 구성합니다. 초반에는 token-id 기반 routing으로 싸게 처리하고, 이후에는 hidden representation 기반 top-k routing을 쓰며, selection bias는 expert 선택에만 쓰고 최종 weight 계산은 bias 없는 score에서 다시 가져옵니다.", runtime: "초반 hash layer는 token id로 expert id를 고르고, 이후 layer는 score top-k로 고릅니다." },
    "routed-experts": { why: "선택된 expert별로 token rows를 모아 FP4 SwiGLU FFN을 실행합니다. 각 expert 내부에서는 silu(w1(x)) * w3(x) 후 w2로 hidden dimension을 복원하므로, token당 compute를 제한하면서 큰 expert pool의 parameter capacity를 조건부로 사용할 수 있습니다.", runtime: "Pro는 384 routed expert, token당 6 activated expert를 사용합니다." },
    "shared-expert": { why: "sparse routing과 무관하게 모든 token에 공통 SwiGLU FFN 경로를 제공하고, 그 결과를 routed expert output에 항상 더합니다. 공통 변환을 흡수하는 역할로 볼 수 있지만, routed expert load를 실제로 얼마나 완화하는지는 공개 코드만으로 단정하기 어렵습니다.", runtime: "모든 token에 대해 계산되고 routed expert 누적값 뒤에 더해집니다.", ui: "routing gate 뒤가 아니라 routed expert와 병렬인 always-on path로 그립니다.", open: "공통 변환을 흡수해 routed expert 부담을 줄일 수 있다는 해석은 가능하지만, 공개 inference 코드만으로 load-balance 메커니즘이라고 단정할 수는 없습니다." },
    "hc-post-moe": { why: "MoE residual mixing과 output injection을 합쳐 다음 decoder block이 받을 [B,S,4,D] residual state를 만듭니다. 이 writeback 결과가 다음 block input이 되므로, sparse expert update와 기존 residual lane transport가 같은 state로 정리됩니다." },
    head: { why: "모든 decoder layer가 끝난 뒤 mHC lane collapse, final norm, last-token vocab projection을 output-only stage로 묶습니다. 공식 path는 x[:, -1]만 logits 계산에 사용하므로, LM head가 매 layer나 모든 token에서 반복 실행되는 구조가 아닙니다.", runtime: "공식 get_logits는 x[:, -1]만 사용하므로 마지막 token만 projection합니다." },
    mtp: { why: "최종 stack state 뒤에서 auxiliary next-token prediction을 수행해 학습/추론 보조 경로를 제공합니다. 이 branch는 token embedding path와 hidden path를 결합하지만, MTP branch의 정확한 serving 사용 범위는 공개 코드 해석이 더 필요합니다.", runtime: "embedding/head module을 재사용하고 SWA-only attention mode를 가집니다." },
    logits: { why: "최종 token distribution을 만들기 위한 vocabulary score vector를 제공합니다. 이 노드는 architecture graph의 마지막 score 생성 단계이고, sampling이나 top-p 같은 decoding policy는 모델 구조 밖 runtime 단계입니다.", ui: "sampling, top-p, tool decoding은 이 architecture graph 밖의 단계입니다." },
    ...
    "hc-controller": { why: "token별 residual lane 상태에서 pre, post, comb를 한 번에 예측하는 작은 controller를 둡니다. flatten된 lane에는 controller linear 전후의 RMS scale 정규화가 들어가며, 24-channel 출력은 attention 계산 자체와 residual mixing 정책을 분리해 안정성과 표현력을 함께 확보합니다.", runtime: "flatten된 lane은 controller linear 출력 전후로 RMS scale 정규화가 들어갑니다.", ui: "pre, post, comb 세 갈래로 split되는 지점으로 보여줍니다." },
    "hc-split": { why: "controller output을 read coefficient, write coefficient, residual lane mixing matrix라는 서로 다른 의미의 텐서로 명확히 분해합니다. 공식 kernel index 기준으로 0:4는 pre, 4:8은 post, 8:24는 comb이므로 이후 edge shape과 data dependency가 pre/post/comb로 깔끔하게 나뉩니다." },
    "hc-pre-sigmoid": { why: "read weight를 양수 범위로 제한해 lane weighted sum이 무제한 부호/scale로 흔들리지 않게 합니다. scaled sigmoid와 epsilon으로 만든 pre coefficient는 hc_read에서 lane 축 가중합에 직접 쓰이므로, attention 입력 stream의 scale을 안정적으로 잡아 줍니다." },
    "hc-post-sigmoid": { why: "sublayer output을 4개 lane에 주입할 때 lane별 강도를 bounded coefficient로 제어합니다. pre와 달리 +eps 없이 2 * sigmoid 형태를 쓰며, 이 값이 attention output writeback의 scale과 lane placement를 제한합니다." },
    "hc-comb-softmax": { why: "4x4 comb raw logits를 먼저 row-wise probability-like matrix로 만들어 Sinkhorn 반복의 출발점을 안정화합니다. 이 단계의 출력은 Sinkhorn 반복 전 초기 row-normalized matrix이며, epsilon까지 더해 다음 row/column normalization이 다룰 matrix를 안정적으로 만듭니다." },
    "hc-comb-sinkhorn": { why: "comb matrix를 row/column 양쪽에서 반복 정규화해 residual lane transport가 특정 lane으로 붕괴하지 않게 합니다. Sinkhorn을 거친 최종 4x4 matrix가 실제 residual lane mixing에 들어가며, 이 doubly stochastic에 가까운 lane transport가 gradient와 signal propagation을 안정화하는 역할을 합니다." },
    "hc-sinkhorn": { why: "controller logit을 실제 data path coefficient로 바꾸면서 comb에는 doubly stochastic 제약을 걸어 residual lane 이동을 안정화합니다. split과 Sinkhorn을 거치면 pre/post는 [B,S,4] vector로, comb는 [B,S,4,4] matrix로 정리되어 read/write/mix 계수가 안정적인 data path coefficient가 됩니다.", runtime: "comb는 hc_split_sinkhorn kernel에서 row/column normalization을 반복해 doubly stochastic에 가깝게 만듭니다.", ui: "pre/post는 vector, comb는 4x4 heatmap처럼 보여주면 좋습니다.", open: "mHC constraint가 학습 안정성에 기여하는 정확한 분석은 technical report와 같이 대조해야 합니다." },
    "hc-read": { why: "4-lane residual state를 attention이 처리할 단일 hidden stream으로 읽습니다. 연산은 lane axis에 대한 sum(pre * X)이고, pre coefficient로 lane weighted sum을 만들기 때문에 attention compute를 4배로 늘리지 않고 mHC 표현력을 유지합니다.", ui: "controller coefficient가 data path에 처음 적용되는 지점입니다." },
    "attn-residual-mix": { why: "attention output과 별개로 기존 residual lane 자체를 comb matrix로 운반하는 attention writeback의 핵심 residual lane mixing입니다. identity residual보다 lane 간 정보 이동을 허용하므로, attention output injection과 분리된 안정적인 residual transport 항으로 볼 수 있습니다.", ui: "attention output injection과 분리해서 보여줘야 합니다." },
    "attn-post-inject": { why: "attention output 하나를 post coefficient로 4개 lane에 배분해 sublayer 결과가 어느 lane에 남을지 조절합니다. 이 항은 residual lane mixing과 별도로 더해지는 output injection이며, post write path가 residual update의 scale과 lane placement를 제어합니다.", ui: "하나의 attention stream이 4개 lane으로 fan-out되는 구조입니다." },
    "hc-write": { why: "comb로 운반한 기존 residual lane mixing 결과와 post로 주입한 attention output injection을 합쳐 다음 residual lane state를 만듭니다. 즉 writeback은 residual lane mixing + sublayer output injection이며, 이 조합으로 residual 안정성과 attention update 표현력을 동시에 확보합니다." },
    "ffn-hc-flatten": { why: "MoE controller가 현재 4-lane residual 상태 전체를 보고 read/write/mix coefficient를 만들게 합니다. attention mHC와 별도의 controller path를 두어 MoE 입력과 residual update를 독립적으로 조절합니다." },
    "ffn-hc-controller": { why: "MoE sublayer 전용 pre/post/comb logits를 attention과 독립적으로 생성합니다. 구조는 attention controller와 같지만 parameter set이 분리되어 있어, sublayer별 lane mixing 정책을 따로 학습할 수 있습니다.", ui: "attention controller와 같은 구조지만 독립 parameter라고 표시합니다." },
    "ffn-hc-split": { why: "MoE-side controller output을 read, write, comb 텐서로 분해해 router/expert data path에 각각 다른 coefficient를 공급합니다. attention-side와 같은 kernel 구조를 쓰지만 별도 parameter set을 사용하므로, MoE path의 dependency가 read/write/comb로 독립적으로 나뉩니다." },
    ...
    "ffn-hc-sinkhorn": { why: "MoE sublayer의 controller output을 data path coefficient로 변환하면서 attention 쪽과 같은 mHC 제약을 적용합니다. 여기서 MoE writeback에 들어갈 residual lane mixing weights가 나오며, attention 쪽과 동일한 4x4 comb matrix 제약으로 MoE에서도 residual lane stability가 유지됩니다.", ui: "attention 쪽과 동일한 4x4 comb matrix 시각화를 재사용합니다." },
    "hc-pre-moe": { why: "router와 experts가 처리할 단일 hidden stream을 4-lane residual에서 읽어냅니다. 연산은 lane axis에 대한 sum(pre * residual)이고, lane을 먼저 단일 hidden stream으로 접기 때문에 MoE compute를 4개 lane마다 반복하지 않습니다." },
    "ffn-residual-mix": { why: "MoE sublayer를 지나도 기존 residual lane 정보를 comb matrix로 계속 운반합니다. attention writeback과 같은 residual lane mixing 구조지만 별도 mHC weights를 쓰므로, sparse expert update와 별개로 MoE 쪽 residual signal을 안정적으로 보존합니다." },
    ...
    "kv-slice": { why: "shared KV 안에서 position-aware key slice와 content/value slice를 분리해 K와 V가 서로 다른 의미를 갖게 합니다. K score에는 RoPE slice가 필요하지만 V sum에는 position phase가 없는 shared value가 필요하므로, 448/64 split으로 RoPE를 필요한 key slice에 집중시킵니다.", runtime: "RoPE는 마지막 64 dim에만 들어가고, 448 dim은 content/value semantics를 유지합니다." },
    ...
    "swa-prefill-write": { why: "prefill 중에도 최근 local token을 압축하지 않고 보존해 short-range attention 품질을 지킵니다. 긴 prompt에서는 window 밖 token을 compressed path가 담당하고, 128-window cache는 compressed memory의 정보 손실을 local window로 보완합니다.", runtime: "SWA window 밖의 오래된 token은 compressed path가 담당합니다." },
    "swa-decode-write": { why: "decode에서 새 token KV를 고정 크기 128-slot ring buffer에 갱신해 uncompressed local cache memory를 상수로 유지합니다. runtime cache manager가 start_pos % 128 위치에 쓰면서 오래된 uncompressed entry를 자연스럽게 덮어쓰므로, bounded memory로 local attention을 유지합니다.", runtime: "slot은 start_pos % 128이며 오래된 uncompressed KV는 자연스럽게 덮어씁니다." },
    ...
    "comp-ape": { why: "compressor-local block 내부 위치 정보를 gate score에 더해 pooling이 순서/위치를 완전히 잃지 않게 합니다. attention RoPE와는 별개로 softmax gate에 block 내부 상대 위치를 알려 주는 path이며, compressed entry의 block-local 위치 민감도를 보완합니다.", runtime: "attention RoPE와 별개인 compressor-local score path입니다." },
    "comp-cutoff": { why: "현재 chunk와 이전 tail을 합친 뒤 완성된 block만 compressed cache로 보내고 나머지는 tail로 넘깁니다. decode에서는 대부분 remainder가 누적되다가 block boundary에서만 compressed entry가 생기며, 이 state 흐름이 chunking과 decode step 크기에 따른 결과 흔들림을 줄입니다.", runtime: "block boundary에 닿지 않으면 compressed cache write가 발생하지 않을 수 있습니다." },
    ...
    "comp-block-view": { why: "projection 결과를 pooling kernel이 읽기 쉬운 block/token/value 축으로 바꿔 c4a overlap과 c128a block 처리를 같은 추상화에 올립니다. CSA에서는 overlap 후 span=8처럼 해석되고 HCA에서는 128-token block으로 해석되므로, reshape와 overlap 처리가 compressor compute를 block/token/value 축에 맞춥니다.", runtime: "CSA는 overlap 때문에 span이 8처럼 보이고 HCA는 128-token block이 됩니다." },
    ...
    "comp-anchor": { why: "compressed block 전체에 하나의 대표 position을 부여해 RoPE와 causal indexing이 가능한 cache entry로 만듭니다. block 내부 token마다 position을 따로 주지 않고 stride별 anchor id를 쓰기 때문에, compressed entry는 cache에서 다루기 쉬워지는 대신 block 내부 세부 위치는 일부 손실될 수 있습니다.", runtime: "c4a는 stride 4 anchor, c128a는 stride 128 anchor를 씁니다." },
    ...
    "route-bias": { why: "expert 선택 확률만 조정하고 실제 mixture weight는 원래 affinity score에서 계산해 output magnitude 왜곡을 줄입니다. bias는 top-k selection score에만 더해지고, weight 계산에는 bias 없는 original score를 다시 gather하므로 selection correction과 output weighting이 분리됩니다.", runtime: "bias는 topK에만 들어가고 normalize weight는 original score gather로 계산합니다." },
    ...
    "expert-counts": { why: "expert별 token row 수를 세어 dispatch/scatter kernel sizing과 병렬 실행 크기를 정하는 runtime metadata를 만듭니다. 이 count는 routing 실행을 관리하는 데 쓰이고, 동시에 expert별 load imbalance를 관찰할 수 있는 지점이 됩니다.", runtime: "load imbalance를 관찰할 수 있는 지점이기도 합니다." },
    ...
    "moe-allreduce": { why: "tensor/expert parallel 환경에서 shard별 MoE 결과를 동일한 hidden stream으로 합칩니다. 단일 GPU 개념 그래프에서는 identity처럼 보일 수 있지만, distributed inference에서는 all-reduce가 분산 shard의 MoE output을 맞추는 명시적 동기화 지점입니다.", runtime: "single-device conceptual graph에서는 거의 identity처럼 보일 수 있습니다." },
    ...
    "attn-inv-rope": { why: "shared KV가 key로 쓰일 때 들어간 RoPE phase가 value output에 남는 문제를 보정합니다. K score에는 RoPE가 필요하지만 V sum에서는 위치 phase를 제거해야 하므로, inverse RoPE path가 KV sharing의 memory 이득을 유지하면서 value phase 부작용을 줄입니다.", runtime: "K에는 RoPE가 필요하지만 V에는 위치 회전이 직접 섞이면 부자연스럽기 때문에 output path에서 보정합니다." },
    ...
    "last-token": { why: "autoregressive decode에서 필요한 마지막 token hidden만 vocab projection해 output compute를 줄입니다. 모든 layer나 모든 token에서 LM head를 매번 계산하는 구조가 아니며, x[:, -1]만 projection하는 이 경계가 그래프의 output path 의미를 명확히 합니다.", runtime: "이 노드가 'LM head가 매 layer마다 돈다'는 오해를 막는 핵심 경계입니다." },
    ...
    "stack-entry": { why: "입력 처리와 반복 decoder layer 내부를 분리해, input과 embedding이 매 layer 안에 다시 생기는 것처럼 보이는 오해를 막습니다. 이 노드는 반복 block 내부의 두 번째 input stream이 아니라 대표 decoder layer로 들어가는 최종 stack state의 시작점을 표시합니다.", ui: "레이어 모드 버튼은 대표 layer 내부 경로만 바꾸며 input node 자체를 바꾸는 것이 아닙니다." },
    "stack-exit": { why: "모든 decoder layer를 지난 최종 residual lane state와 output head를 분리합니다. 공식 path는 every layer가 아니라 전체 stack이 끝난 뒤 x[:, -1]에서 logits를 계산하므로, 이 노드가 per-layer LM head처럼 보이는 오해를 줄입니다.", ui: "LM head가 매 layer마다 실행되는 것처럼 보이는 오해를 막습니다." },
    "hca-all-compressed": { why: "R=128처럼 매우 강하게 압축된 layer에서는 Lightning indexer를 쓰지 않고 valid compressed block 전체를 읽어 retrieval overhead를 없앱니다. HCA mode에서는 Attention.indexer가 없으므로, compressed memory 전체와 SWA window에 attend하는 구조로 보는 편이 맞습니다.", runtime: "HCA에는 Lightning indexer가 없고 R=128에서는 Attention.indexer가 None입니다.", ui: "compressed memory 전체 + SWA에 attend하는 구조로 표시합니다." },
  }).forEach(([id, details]) => {
